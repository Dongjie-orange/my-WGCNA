...
#----------------------------------------------------------------------------------
#  Step 1: Read the gene counts table and plot the sample tree
#----------------------------------------------------------------------------------

# if the input is counts
if(F){
# Read the gene counts table
data0 <- read.table("/Users/djchen/Desktop/WGCNA_tutorial-main/gene_counts_table_WGCNA_LC.txt",header=T,row.names=1,sep="\t")
# # LH38_Z1_1 LH38_Z1_2 LH38_Z1_3 LH38_Z2_1 LH38_Z2_2
# # Zm00001d001763       135       202       113       136       230
# # Zm00001d001766      5540      2132      6857      7741      3677
# # Zm00001d001770         1         0         0         0         5
# # Zm00001d001774      6517      2393      6626      8215      4183
# Zm00001d001775         0         0         0         0         0
# Normalization with log2(FPKM+1)
sample_metadata <- read.csv(file = "/Users/djchen/Desktop/WGCNA_tutorial-main/sample_info.csv")
# sample_ID Genotype Zone Replicate
# 1 X787_Z1_1      787   Z1         1
# 2 X787_Z1_2      787   Z1         2
# 3 X787_Z1_3      787   Z1         3
# 4 X787_Z2_1      787   Z2         1
# 5 X787_Z2_2      787   Z2         2
# 6 X787_Z2_3      787   Z2         3
...
# the input is tpm/fpkm/vst
datExpr0 <- t(log2(input_exp+1))

# reshuffle the data
m.mad <- apply(datExpr0,1,mad)
datExpr0 <- datExpr0[which(m.mad > max(quantile(m.mad, probs=seq(0, 1, 0.25))[2],0.01)),] # filter top 75% & mad > 0.001
#transposition
datExpr0 <- t(datExpr0)
#check the NA
...
abline(h = cut_num,col ="red")#划定需要剪切的枝长
dev.off()
# remove the outlier
cut_num <- 190
clust <- cutreeStatic(sampleTree,cutHeight = cut_num,minSize =10)
table(clust)
keepSamples <- (clust == 1)
datExpr <- datExpr0[keepSamples,]
nGenes <- ncol(datExpr) # 14941
nSamples <- nrow(datExpr) # 209

#----------------------------------------------------------------------------------
#  Step 2: Choose soft threshold parameter
#----------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------
#  Step 3: Turn data expression into topological overlap matrix
#----------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------
#  Step 5:  Export of networks to external software
#----------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------
#  Step 6:   Correlate module eigen-genes and samples (or other discrete data)
#----------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------
#  Step 7:    Correlation between gene modules and microbial traits (continuous data)
#----------------------------------------------------------------------------------
...
moduleTraitCor <- or(MEs, bac_traits, use = "p");
...
#----------------------------------------------------------------------------------
#  Step 8: Intramodular analysis: identifying genes with high geneModuleMembership & geneTraitSignficance
#----------------------------------------------------------------------------------
